...
class Cell(FixedAgent):
    ...
    def determine_state(self):
        ...
        #added the moduled so the borders are connected (torus)
        upLeft =[(self.x - 1) % 50, (self.y + 1) % 50] 
        up = [(self.x) % 50, (self.y + 1) % 50]
        upRight = [(self.x + 1) % 50, (self.y + 1) % 50] 
        ...
        #now we apply the rules based on the status of the upper neighbors
        if upLeftStatus and upStatus and upRightStatus:
