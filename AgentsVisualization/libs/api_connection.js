...
// enviar diccionario para que se inicie el servidor 
...
async function getAgents() {
    try {
        ...
        if (response.ok) {
            ...
            if (agents.length == 0) {
                //el primer for que crea los agentes
                ...
                // este es para que cuando ya hayan agentes, solo actualice su posicion
                for (const agent of result.positions) {
 ...
 */
// la simulación sigue hacinedo esta funcion todo el rato, step del modelo
async function update() {
    try {
        ...
        if (response.ok) {
            ...
            // agarra el segundo for de getAgents
            await getAgents();
